...
<tr><td> <em>Response:</em> <p>This had to be accounted for becuase both the challenge list and navigation<br>list<br>share the nested ul li elements so using ul and li as selectors<br>would affect both. One way to achieve this is by specifically selecting elements<br>through the nav selector or directly selecting the elements. Another way that, I<br>actually used, to make the styling for the challenge list not impact the<br>navigation list is through the :not() selector. This is done by putting the<br>ul li :not(a) selectors. This selects li elements inside ul elements but excludes<br>elements. All of navigation are elements so this effectively excludes nav.<br></p><br></td></tr>
...
<tr><td> <em>Response:</em> <p>This was done the same way as the h1 tag described in Sub-Task<br>9 except using the title tag instead. Again with a timeout to allow<br>time for the page to update.</p><br><br></td></tr>
</table></td></tr>
