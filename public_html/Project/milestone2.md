...
<tr><td> <em>Generated: </em> 4/18/2022 11:56:29 PM</td></tr>
...
<tr><td> <em>Response:</em> <p>When a user with an admin role is viewing them, edit buttons are<br>added in-line with each product in the tables on the Shop Page and<br>List Items Page. This was done through a PHP if statement within the<br>table checking if the user is an admin and adding in an additional<br>column header to edit the product, along with edit links on each row.<br>The link fields of these <a> tags were filled with a link to<br>the Product_Edit Page with the item id sent as a get request through<br>the url (product_edit.php?id=). Once they arrive on the page, if the user is<br>not an admin or the product does not exist it will bounce them<br>from the page to the homepage with an error message. </p><br><p>With the Edit<br>Product Page only being accessible from specific products, either in rows or on<br>product pages, it will always have a populated $_GET populated with the product&#39;s<br>id. So the Edit Product&#39;s Page pulls from the database (using SQL and<br>PDO through PHP) all the relevant values from the database and populates the<br>form with the previous values. This allows the admin easier ability to edit<br>them. When the editing is completed, it can be submitted with a POST<br>request. There is validation for each value within the HTML form as well<br>as server side through PHP, and if it passes all these steps, it<br>is prepared through placeholders into an SQL query. This is an UPDATE products<br>SET that updates the row that matches the item_id.<br><br></p><br></td></tr>
